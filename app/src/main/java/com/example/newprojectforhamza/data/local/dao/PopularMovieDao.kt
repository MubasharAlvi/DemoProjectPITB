...
@Dao
interface PopularMovieDao : BaseDao<PopularResultDTO> {

    @Query("DELETE FROM popularMovies")
    suspend fun clear()

    @Query("SELECT * FROM popularMovies ORDER BY popularity DESC")
    fun observePopular(): Flow<List<PopularResultDTO>>

    @Query("SELECT * FROM popularMovies ORDER BY voteAverage DESC")
    fun observeTopRated(): Flow<List<PopularResultDTO>>

    @Query("SELECT * FROM popularMovies ORDER BY popularity DESC")
    fun pagingSource(): PagingSource<Int, PopularResultDTO>
}